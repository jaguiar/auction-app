...
import org.yaourtcorp.auction.exception.NoBidOverReservePriceException
import org.yaourtcorp.auction.exception.MoreThanOneWinnerException

@RunWith(classOf[JUnitRunner])
class AuctionApplicationWithPoorMapVersionSpec extends AbstractUnitSpec with BeforeAndAfter {
  ...
  "AuctionApplicationWithPoorMapVersion" should "throw a NoBidOverReservePriceException when there is no buyer" in {
    //test
    val inputStream = new ByteArrayInputStream(zeroBuyerCase.getBytes());
    intercept[NoBidOverReservePriceException] { 
      Console.withOut(printStream)({
       Console.withIn(inputStream)(AuctionApplicationWithPoorMapVersion.main(Array.empty))
     })
    }
  }
